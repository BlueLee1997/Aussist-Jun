...
function getNextOpeningTime(periods: { open: { day: number; time: string } }[]): string | null {
  ...
  if (todayPeriod) {
    ...
    if (currentHHMM < openHHMM) {
      return todayPeriod.open.time; // e.g. "0800"
    }
  }
  // 2) 오늘 지났거나 데이터 없으면 다음 요일로
  for (let i = 1; i <= 7; i++) {
    const day = (today + i) % 7;
    const next = periods.find(p => p.open.day === day);
    if (next) {
      return next.open.time;
    }
  }
  return null;
}
...
function getClosingTime(periods: { open?: { day: number; time?: string }; close?: { day: number; time?: string } }[]): string | null {
  ...
  if (todayPeriod && todayPeriod.open && todayPeriod.close && todayPeriod.open.time && todayPeriod.close.time) {
    const openHHMM = parseInt(todayPeriod.open.time, 10);
    const closeHHMM = parseInt(todayPeriod.close.time, 10);
    if (currentHHMM >= openHHMM && currentHHMM < closeHHMM) {
      return todayPeriod.close.time;
    }
  }
  return null;
}
...
export default function Hospital() {
  ...
  const handleSearch = async () => {
    ...
    setLoading(true);
    try {
      const res = await searchPlaces(searchText, region?.latitude, region?.longitude);
      setHospitals(res.results || []);
      if (res.results?.length) {
        const f = res.results[0].geometry.location;
        setRegion({ latitude: f.lat, longitude: f.lng, latitudeDelta: 0.05, longitudeDelta: 0.05 });
      }
    } catch {
      setHospitals([]);
    }
  ...
  return (
    <GestureHandlerRootView style={styles.container}>
      <Animated.View style={[styles.mapContainer, { bottom: Animated.add(bottomOffset, animatedHeight) }]}>
        <MapView
          style={styles.map}
      ...
      <View style={[styles.searchBoxWrap, { top: insets.top + 10 }]}>
      ...
      <Animated.View style={[styles.hospitalListContainer, { bottom: bottomOffset, height: animatedHeight }]}>
        <View style={styles.hospitalListHeader}>
          <Text style={styles.hospitalListTitle}>Hospitals Near You</Text>
