          ...
          <View style={{ marginBottom: 24 }}>
            ...
            {facilities.map((fac, idx) => (
              <View key={idx} style={{
                ...
                <Image source={{ uri: fac.image }} style={{ width: '100%', height: 120 }} />
                <View style={{ padding: 12 }}>
                  <TText style={{ fontWeight: 'bold', fontSize: 16, marginBottom: 4 }}>{fac.name}</TText>
                  <TText style={{ color: '#6B7280', marginBottom: 4 }}>{fac.type}</TText>
                  <TText style={{ color: '#6B7280', marginBottom: 8 }}>{fac.address}</TText>
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                    <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
                      {fac.services.map((service, i) => (
                        <View key={i} style={{
                          ...
                          <TText style={{ fontSize: 12 }}>{service}</TText>
                        </View>
                      ))}
                    </View>
                    <TText style={{ color: theme.colors.primary }}>{fac.distance}</TText>
                  </View>
                </View>
              </View>
            ))}
